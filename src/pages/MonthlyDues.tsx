...
import { Plus, CheckCircle2, Circle, HandCoins, Calendar as CalendarIcon } from 'lucide-react'
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className="p-3 bg-blue-600 text-white rounded-2xl shadow-lg shadow-blue-500/20 active:scale-95 transition-transform"
        >
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className={`transition-colors ${due.isPaid ? 'text-green-500' : 'text-gray-300 dark:text-gray-600'}`}
                    >
                      {due.isPaid ? <CheckCircle2 size={28} strokeWidth={2.5} /> : <Circle size={28} strokeWidth={2.5} />}
                    </button>
                    <div>
                      <h3 className={`font-black text-sm ${due.isPaid ? 'text-green-700 dark:text-green-400' : ''}`}>
                        {due.label}
                      </h3>
                  ...
                  <div className="text-right">
                    <p className={`font-black ${due.isPaid ? 'text-green-600' : ''}`}>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-end px-1">
                     ...
                     <p className={`text-[10px] font-black uppercase ${due.isPaid ? 'text-green-600' : 'text-blue-600'}`}>
                       {Math.round(progress)}%
                     </p>
                  </div>
                  <div className="h-3 w-full bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      className={`h-full transition-colors ${due.isPaid ? 'bg-green-500' : 'bg-blue-600'}`}
                    />
                  </div>
                  {!due.isPaid && (
                    <button
                      onClick={() => setIsContributing(due.id)}
                      className="w-full mt-2 py-2 bg-gray-50 dark:bg-gray-700/50 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-[10px] font-black uppercase rounded-xl flex items-center justify-center gap-2 transition-colors"
                    >
